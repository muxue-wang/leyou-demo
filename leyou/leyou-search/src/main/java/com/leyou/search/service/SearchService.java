...
public class SearchService {
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        skus.forEach(sku -> {
            ...
            //获取sku中的图片，数据库的图片可能是多张，多张以","分割，返回图片数组，获取第一张图片
            map.put("image",StringUtils.isBlank(sku.getImages())?"":StringUtils.split(sku.getImages(),",")[0]);
            skuMapList.add(map);
        });
        //根据spu中的cid3查出所有的规格参数
        ...
        params.forEach(param -> {
            ...
            if (param.getGeneric()){
                ...
                //如果是特殊规格
                final List<Object> value = specialSpecMap.get(param.getId().toString());
                spec.put(param.getName(),value);
            }
        });
    ...
    public void save(Long id) throws IOException {
        final Spu spu = this.goodsClient.querySpuById(id);
        final Goods goods = this.buildGoods(spu);
